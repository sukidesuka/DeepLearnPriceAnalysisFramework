...
class InvestingSilverPriceScanner(ScriptTask):
    ...
    def run(self):
        ...
        while True:
            ...
            for row in df.values:
                ...
                self.global_share.price_db.insert(row[0], 'silver', 'price', row[1], save_to_file=False)
                self.global_share.price_db.insert(row[0], 'silver', 'open', row[2], save_to_file=False)
                self.global_share.price_db.insert(row[0], 'silver', 'high', row[3], save_to_file=False)
                self.global_share.price_db.insert(row[0], 'silver', 'low', row[4], save_to_file=False)
                self.global_share.price_db.insert(row[0], 'silver', 'vol', row[5], save_to_file=False)
                self.global_share.price_db.insert(row[0], 'silver', 'change', row[6], save_to_file=False)
