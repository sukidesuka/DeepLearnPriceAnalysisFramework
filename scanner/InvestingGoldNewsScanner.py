...
class InvestingGoldNewsScanner(ScriptTask):
    ...
    def run(self):
        ...
        while True:
            # 發出爬取請求
            task = self.global_share.scanner_manager.ask_task('https://www.investing.com/commodities/gold-news/{}'.
            ...
            if task.response.status_code == 302:
                i = 1
                continue
            self.global_share.log.write('page{} gold success'.format(i))
            html = etree.HTML(task.response.text)
            # 一般來講，每頁文章是10個
            for x in range(1, 11):
                _title = html.xpath(
                    '/html/body/div[6]/section[1]/div[8]/article[{}]/div[@class="textDiv"]/a/@title'.format(x))
                _from = html.xpath(
                    '/html/body/div[6]/section[1]/div[8]/article[{}]/div[@class="textDiv"]/*['
                ...
                _time = html.xpath(
                    ...
                    '@class="articleDetails"]/span[2]/text()'.format(
                        x))
                _link = html.xpath(
                    '/html/body/div[6]/section[1]/div[8]/article[{}]/div[@class="textDiv"]/a/@href'.format(x))
                _text = html.xpath(
                    '/html/body/div[6]/section[1]/div[8]/article[{}]/div[@class="textDiv"]/p/text()'.format(x))
                ...
                # 將獲取到的内容寫入到數據庫
                self.global_share.text_db.insert(_time, 'gold', _title, _from, _text)
